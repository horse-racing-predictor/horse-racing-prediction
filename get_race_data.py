...
if (argc != 3):   # 引数が足りない場合は、その旨を表示
    print ('Usage: # %s [year] [race_place]'%argvs[0])
    quit()         # プログラムの終了

for num in range(1,5):
	for day in range(1,9):
		for racenum in range(1,12):
			#url = "http://db.netkeiba.com/race/201808020611/"
			url = "http://db.netkeiba.com/race/"+argvs[1]+argvs[2].zfill(2)+str(num).zfill(2)+str(day).zfill(2)+str(racenum).zfill(2)+"/"
			...
			for l in trlist:
				...
				fname = argvs[1]+argvs[2].zfill(2)+str(num).zfill(2)+str(day).zfill(2)+str(racenum).zfill(2)+".csv"
				f = codecs.open(fname, 'a','cp932') # 書き込みモードで開くcp932
				write=''
				j = 0
				#print(elems)
				while j < len(elems):
